...
module.exports = function (RED) {
  ...
  function HTTPRequest (n) {
    ...
    let node = this;
    let nodeUrl = n.url;
    let nodeFollowRedirects = n['follow-redirects'];
    let isTemplatedUrl = (nodeUrl || '').indexOf('{{') !== -1;
    let nodeMethod = n.method || 'GET';
    let tlsNode;
    if (n.tls)
      tlsNode = RED.nodes.getNode(n.tls);
    ...
    this.on('input', function (msg) {
      ...
      if (msg.payload && (['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)))
  ...
  RED.nodes.registerType('http-request-extended', HTTPRequest, {
    credentials: {
      user: {
        type: 'text'
      },
